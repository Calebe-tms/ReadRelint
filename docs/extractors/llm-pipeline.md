...
> O antigo "Pass 1 legado" (chamada genérica via `rule.get_schema_model()`, tipicamente `IncidentReport`) foi removido. `relint_type` e `main_fact` (determinístico/derivado, fora do escopo do motor LLM) e `participants` (o mais complexo, pendente de desenho próprio) ainda ficam temporariamente sem extração dedicada — com os defaults do Pydantic (`"Outros"`/vazio) até serem reconstruídos, seguindo o plano em [`../proposals/eliminacao-pass1-legado.md`](../proposals/eliminacao-pass1-legado.md). `registry_number`/`registry_agency`/`registry_year` (Pass dedicado acima), `date_of_fact`/`time_of_fact` (100% determinístico, incondicional em `EtlService`) e `location_types` (campo extra no Pass 2) já foram reconstruídos. `participants` **não** cai automaticamente no fallback determinístico por regex (`extract_fallback_participants`) — esse fallback em `EtlService` é reservado exclusivamente ao modo 100% sem-IA (`extraction_method == "Regex (Sem IA)"`), para manter as duas chamadas (LLM e determinística) totalmente separadas.
