...
| `relint_type` | ✅ **Implementado** — `classify_relint_type()` (`backend/engine/cleaners/bm_classifier.py`), mesmo padrão de 2 camadas do `classify_bm_group()` (filename+assunto primeiro, conteúdo como fallback), chamado incondicionalmente em `EtlService.process_file()` para os dois motores. Diferença importante em relação a `classify_bm_group()`: o fallback quando nenhum gatilho bate é **"Ocorrência"**, não "Outros" — é o caso majoritário real (boletim de ocorrência comum), enquanto "Outros" fica reservado pro `bm_group`, onde de fato é o caso majoritário. | Enum fechado de 4 opções (`Ocorrência`, `Disk Denúncia`, `Resposta a PB`, `Outros`) com vocabulário-gatilho previsível no assunto ("DISQUE DENÚNCIA", "RESPOSTA PB..."). |
...
- [x] Implementar `classify_relint_type()` (determinístico), chamado incondicionalmente em `EtlService.process_file()` para os dois motores.
