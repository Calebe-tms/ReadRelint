...
5. O antigo Pass 1 legado (schema genérico `IncidentReport`) foi removido. `registry_number`, `date_of_fact`, `participants` e outros campos que só ele fornecia ficam temporariamente sem extração dedicada, sendo reconstruídos campo a campo — ver detalhamento completo em [`../extractors/llm-pipeline.md`](../extractors/llm-pipeline.md) e o plano em [`../proposals/eliminacao-pass1-legado.md`](../proposals/eliminacao-pass1-legado.md).
