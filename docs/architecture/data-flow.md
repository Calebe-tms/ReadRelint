...
5. **Pass dedicado — Registro Policial em Outro Órgão** (`extractors/registry_extractor.py`): resolve `registry_number`/`registry_agency`/`registry_year`, restrito ao corpo narrativo pós-`ANEXOS:`, com reclassificação determinística por contagem de dígitos.
6. **Pass dedicado — Participantes** (`extractors/llm_participants_extractor.py`): resolve `name`/`participation_type`/`background` via LLM; `nickname`/`document` são resolvidos deterministicamente por proximidade ao nome, reaproveitando as mesmas funções já maduras do motor determinístico (seção 5b, abaixo).

O antigo Pass 1 legado (schema genérico `IncidentReport`) foi removido por completo — todos os campos que só ele fornecia (`registry_number`/`registry_agency`/`registry_year`, `date_of_fact`/`time_of_fact`, `relint_type`, `location_types`, `main_fact`, `participants`) já foram reconstruídos, cada um com o tratamento mais adequado (determinístico onde formulaico, pass LLM dedicado onde exige julgamento genuíno) — ver detalhamento completo em [`../extractors/llm-pipeline.md`](../extractors/llm-pipeline.md) e o histórico em [`../proposals/eliminacao-pass1-legado.md`](../proposals/eliminacao-pass1-legado.md) (proposta encerrada).
