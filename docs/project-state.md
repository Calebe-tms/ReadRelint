...
- [x] **Eliminação Total do Pass 1 Legado (6 de 6 campos reconstruídos):** Detalhamento em [`eliminacao-pass1-legado.md`](./proposals/eliminacao-pass1-legado.md), ADR-0096/0099/0100/0104. `RegistryExtractor` (pass LLM dedicado pra registro policial em outro órgão, com reclassificação por contagem de dígitos), `date_of_fact`/`time_of_fact` e `classify_relint_type()` 100% determinísticos, `location_types` no `LocationExtractor`, `main_fact` derivado do resumo, `participants` via `LlmParticipantsExtractor` (ver ADR-0104) — todos aplicados incondicionalmente pros dois motores (LLM e determinístico) onde cabível, com garantia travada por teste de que essa "Fase B" nunca sobrescreve/reprocessa o que a Fase A já resolveu. Proposta encerrada — nenhum campo do Pass 1 legado segue sem substituto.
- [x] **Módulo Gerenciador de Pessoas (Schema + Importação App-AJ):** Detalhamento em [ADR-0101](./adr/0101-gerenciador-pessoas-schema-importacao-app-aj.md), [ADR-0102](./adr/0102-sqlmodel-alembic-modulo-gerenciador-pessoas.md) e [`gerenciador-pessoas-app-aj.md`](./proposals/gerenciador-pessoas-app-aj.md). Novo schema (`qrb`, `grupos_criminosos`, `grupos_familiares`, `veiculos`, `pessoa_fotos` e 4 pivots) via **SQLModel + Alembic** — primeira adoção de ORM/migração versionada no projeto, isolada só a este módulo (motor de RELINT continua SQL puro). Dados reais da planilha externa "App-AJ" importados (1693 pessoas, 87 QRBs, 42 grupos criminosos, 16 grupos familiares, 14 veículos e todos os vínculos) via script de migração única (`scripts/import_app_aj_data.py`). `pessoas.chave_pessoa` eliminada por redundância com `documento`, que agora é a chave única da tabela. Endpoints REST/UI do dashboard e a extração de participantes no motor LLM ficam para os próximos passos.
- [x] **Reorganização da Suíte de Testes por Área:** `tests/` dividido em `motor_llm/`, `motor_regex/`, `modulo_pessoas/` e `dashboard/`, pra permitir rodar só o subconjunto afetado por uma mudança em vez da suíte inteira — ver [`tests/README.md`](../tests/README.md).
- [x] **Correção de Perda de Dados: "Limpar Base & Reprocessar Tudo" Apagava o Gerenciador de Pessoas:** Detalhamento em [ADR-0105](./adr/0105-limpar-base-nunca-afeta-modulo-pessoas.md). `SqliteRepo.clear_all()` fazia `DELETE FROM pessoas;` além de `DELETE FROM relints;` — como as duas tabelas passaram a ser compartilhadas, o botão de reset do painel desktop apagaria as 1693 pessoas importadas da planilha App-AJ. Corrigido: `clear_all()` só limpa `relints` (cascata via FK cuida do resto); chamada redundante `person_repo.clear_all()` removida do controller; diálogo de confirmação atualizado. Teste de regressão travando o comportamento.
...
- [x] **Task 1.5 — Eliminação Total do Pass 1 Legado, Quebrado em Passes Dedicados:**
  - Plano completo em [`eliminacao-pass1-legado.md`](./proposals/eliminacao-pass1-legado.md) e ADR-0096/0099/0100.
  ...
  - `date_of_fact`/`time_of_fact` → 100% determinístico. `relint_type` → novo classificador determinístico (`classify_relint_type()`). `registry_number`/`registry_agency`/`registry_year` → `RegistryExtractor`, novo pass LLM dedicado. `location_types` → integrado ao `LocationExtractor` existente. `main_fact` → derivado sem chamada nova. `participants` → `LlmParticipantsExtractor` (ver ADR-0104) — **último campo reconstruído**, proposta encerrada.

### 👤 FASE 4: MÓDULO GERENCIADOR DE PESSOAS (App-AJ)
- [x] **Task 4.1 — Schema + ORM + Migração de Dados:** Detalhamento em ADR-0101/0102 e [`gerenciador-pessoas-app-aj.md`](./proposals/gerenciador-pessoas-app-aj.md). SQLModel + Alembic, dados reais da planilha App-AJ importados.
- [x] **Task 4.2 — Configurar extração de Pessoas do RELINT:** Detalhamento em [ADR-0104](./adr/0104-llm-participants-extractor-pass-dedicado.md). Novo `LlmParticipantsExtractor` (pass LLM dedicado, `backend/engine/extractors/llm/extractors/llm_participants_extractor.py`) resolve `name`/`participation_type`/`background` via LLM (schema minúsculo `ParticipantsExtraction`), e enriquece `nickname`/`document` deterministicamente por proximidade textual ao nome — reaproveitando `extract_nickname()`/`extract_document_near_name()` (`role_detector.py`) e a blacklist anti-PM/instituição já madura (`is_blacklisted_name()`, `negative_filters.py`) do motor determinístico. Guardrail de evidência literal (`text_contains`) descarta qualquer nome que a LLM não sustente no texto. Persistência (upsert em `pessoas`/`relint_participantes`) reaproveita o pipeline já existente em `EtlService.process_file()`, sem alteração.
- [ ] **Task 4.3 — Dashboard de Gerenciamento de Pessoas (dash + CRUD):** endpoints REST (`/api/v1/pessoas`, `/api/v1/grupos-criminosos`, `/api/v1/grupos-familiares`, `/api/v1/veiculos`, `/api/v1/qrb`, incluindo `PATCH .../dados-aj`) e UI Svelte (lista, dossiê, formulário de edição, editor de pares chave/valor de `dados_aj`).
