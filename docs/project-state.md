...
- [x] **Eliminação Total do Pass 1 Legado (5 de 6 campos reconstruídos):** Detalhamento em [`eliminacao-pass1-legado.md`](./proposals/eliminacao-pass1-legado.md), ADR-0096/0099/0100. `RegistryExtractor` (pass LLM dedicado pra registro policial em outro órgão, com reclassificação por contagem de dígitos), `date_of_fact`/`time_of_fact` e `classify_relint_type()` 100% determinísticos, `location_types` no `LocationExtractor`, `main_fact` derivado do resumo — todos aplicados incondicionalmente pros dois motores (LLM e determinístico), com garantia travada por teste de que essa "Fase B" nunca sobrescreve/reprocessa o que a Fase A já resolveu. Só `participants` segue sem substituto (ver módulo Gerenciador de Pessoas abaixo).
...
- [x] **Task 1.5 — Eliminação Total do Pass 1 Legado, Quebrado em Passes Dedicados:**
  ...
  - `date_of_fact`/`time_of_fact` → 100% determinístico. `relint_type` → novo classificador determinístico (`classify_relint_type()`). `registry_number`/`registry_agency`/`registry_year` → `RegistryExtractor`, novo pass LLM dedicado. `location_types` → integrado ao `LocationExtractor` existente. `main_fact` → derivado sem chamada nova. `participants` → **único campo ainda sem substituto**, passa a depender do módulo Gerenciador de Pessoas (Fase 4, abaixo).
...
- [ ] **Task 4.2 — Configurar extração de Pessoas do RELINT:** completar o último campo do Pass 1 legado (`participants`) — resolver/persistir a pessoa identificada no texto do RELINT em `pessoas` (buscar por documento/nome, criar se não existir). Desenho de pass(es) LLM dedicado(s) ainda a fazer.
